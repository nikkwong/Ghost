...
const productsMetaSchema = require('../../../../../data/schema').tables.products_meta;
...
function handlePostsMeta(frame) {
    let metaAttrs = _.keys(_.omit(productsMetaSchema, ['id', 'product_id']));
    let meta = _.pick(frame.data.products[0], metaAttrs);
    frame.data.products[0].products_meta = meta;
}
...
const forcePageFilter = (frame) => {
    if (frame.options.filter) {
        frame.options.filter = `(${frame.options.filter})+type:product`;
    } else {
        frame.options.filter = 'type:product';
    }
};
...
module.exports = {
    ...
    add(apiConfig, frame, options = {add: true}) {
        ...
        if (_.get(frame,'options.source')) {
            const html = frame.data.products[0].html;

            if (frame.options.source === 'html' && !_.isEmpty(html)) {
                frame.data.products[0].mobiledoc = JSON.stringify(converters.htmlToMobiledocConverter(html));
            }
        }

        frame.data.products[0] = url.forPost(Object.assign({}, frame.data.products[0]), frame.options);

        // @NOTE: force adding post
        if (options.add) {
            frame.data.products[0].type = 'product';
        }

        // CASE: Transform short to long format
        if (frame.data.products[0].authors) {
            frame.data.products[0].authors.forEach((author, index) => {
                if (_.isString(author)) {
                    frame.data.products[0].authors[index] = {
                        email: author
                    };
                }
            });
        ...
        if (frame.data.products[0].tags) {
            frame.data.products[0].tags.forEach((tag, index) => {
                if (_.isString(tag)) {
                    frame.data.products[0].tags[index] = {
                        name: tag
                    };
                }
            });
